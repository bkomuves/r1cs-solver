...
{-# SPECIALIZE solver :: Proxy F16 -> Verbosity -> SymConstraints F16 -> Map Name F16 -> IO [Solution Name F16] #-}
{-# SPECIALIZE solver :: Proxy F20 -> Verbosity -> SymConstraints F20 -> Map Name F20 -> IO [Solution Name F20] #-}
{-# SPECIALIZE solver :: Proxy F24 -> Verbosity -> SymConstraints F24 -> Map Name F24 -> IO [Solution Name F24] #-}

solver :: forall field. Field field => Proxy field 
       -> Verbosity -> SymConstraints field -> Map Name field -> IO [Solution Name field]
solver pxy verbosity (SymConstraints constraints symboltable) initial = 

  do
    unless (prime == fieldPrime pxy) $ do
      error $ "expecting an R1CS over the prime field with p = " ++ show (fieldPrime pxy)
  ...
  where
    ...
    startingRestr :: Restrictions_ field
    ...
    startingPartial_ :: Partial_ field
...
{-# SPECIALIZE substituteLinComb :: Map VarIdx F16 -> LinComb' VarIdx F16 -> LinComb' VarIdx F16 #-}
{-# SPECIALIZE substituteLinComb :: Map VarIdx F20 -> LinComb' VarIdx F20 -> LinComb' VarIdx F20 #-}
{-# SPECIALIZE substituteLinComb :: Map VarIdx F24 -> LinComb' VarIdx F24 -> LinComb' VarIdx F24 #-}
...
{-# SPECIALIZE substituteR1CS :: Map VarIdx F16 -> R1CS' VarIdx F16 -> R1CS' VarIdx F16 #-}
{-# SPECIALIZE substituteR1CS :: Map VarIdx F20 -> R1CS' VarIdx F20 -> R1CS' VarIdx F20 #-}
{-# SPECIALIZE substituteR1CS :: Map VarIdx F24 -> R1CS' VarIdx F24 -> R1CS' VarIdx F24 #-}
...
{-# SPECIALIZE substituteConstraints1 :: (VarIdx, F16) -> [R1CS' VarIdx F16] -> [R1CS' VarIdx F16] #-}
{-# SPECIALIZE substituteConstraints1 :: (VarIdx, F20) -> [R1CS' VarIdx F20] -> [R1CS' VarIdx F20] #-}
{-# SPECIALIZE substituteConstraints1 :: (VarIdx, F24) -> [R1CS' VarIdx F24] -> [R1CS' VarIdx F24] #-}
...
{-# SPECIALIZE substituteLinComb1 :: (VarIdx, F16) -> LinComb' VarIdx F16 -> LinComb' VarIdx F16 #-}
{-# SPECIALIZE substituteLinComb1 :: (VarIdx, F20) -> LinComb' VarIdx F20 -> LinComb' VarIdx F20 #-}
{-# SPECIALIZE substituteLinComb1 :: (VarIdx, F24) -> LinComb' VarIdx F24 -> LinComb' VarIdx F24 #-}
...
{-# SPECIALIZE substituteR1CS_1 :: (VarIdx, F16) -> R1CS' VarIdx F16 -> R1CS' VarIdx F16 #-}
{-# SPECIALIZE substituteR1CS_1 :: (VarIdx, F20) -> R1CS' VarIdx F20 -> R1CS' VarIdx F20 #-}
{-# SPECIALIZE substituteR1CS_1 :: (VarIdx, F24) -> R1CS' VarIdx F24 -> R1CS' VarIdx F24 #-}
...
{-# SPECIALIZE linearSubsLinComb :: (VarIdx, LinComb' VarIdx F16) -> LinComb' VarIdx F16 -> LinComb' VarIdx F16 #-}
{-# SPECIALIZE linearSubsLinComb :: (VarIdx, LinComb' VarIdx F20) -> LinComb' VarIdx F20 -> LinComb' VarIdx F20 #-}
{-# SPECIALIZE linearSubsLinComb :: (VarIdx, LinComb' VarIdx F24) -> LinComb' VarIdx F24 -> LinComb' VarIdx F24 #-}
...
{-# SPECIALIZE linearSubsR1CS :: (VarIdx, LinComb' VarIdx F16) -> R1CS' VarIdx F16 -> R1CS' VarIdx F16 #-}
{-# SPECIALIZE linearSubsR1CS :: (VarIdx, LinComb' VarIdx F20) -> R1CS' VarIdx F20 -> R1CS' VarIdx F20 #-}
{-# SPECIALIZE linearSubsR1CS :: (VarIdx, LinComb' VarIdx F24) -> R1CS' VarIdx F24 -> R1CS' VarIdx F24 #-}
...
{-# SPECIALIZE linearSubsConstraints :: (VarIdx, LinComb' VarIdx F16) -> [R1CS' VarIdx F16] -> [R1CS' VarIdx F16] #-}
{-# SPECIALIZE linearSubsConstraints :: (VarIdx, LinComb' VarIdx F20) -> [R1CS' VarIdx F20] -> [R1CS' VarIdx F20] #-}
{-# SPECIALIZE linearSubsConstraints :: (VarIdx, LinComb' VarIdx F24) -> [R1CS' VarIdx F24] -> [R1CS' VarIdx F24] #-}
...
type Restrictions_ field = Restrictions VarIdx field

printRestrictions_ :: Field f => Restrictions_ f -> IO ()
printRestrictions_ table = 
  forM_ (Map.toList table) $ \(idx,ei) -> do
    putStr $ "  w" ++ show idx ++ " ~> " 
    case ei of
      -- Right (F16 y) -> putStrLn $ show y
      Right y       -> putStrLn $ showF y
...
{-# SPECIALIZE subsIntoRestrictions1 :: (VarIdx, F16) -> Restrictions_ F16 -> Restrictions_ F16 #-}
{-# SPECIALIZE subsIntoRestrictions1 :: (VarIdx, F20) -> Restrictions_ F20 -> Restrictions_ F20 #-}
{-# SPECIALIZE subsIntoRestrictions1 :: (VarIdx, F24) -> Restrictions_ F24 -> Restrictions_ F24 #-}
...
{-# SPECIALIZE linearSubsRestrictions :: (VarIdx, LinComb' VarIdx F16) -> Restrictions_ F16 -> Restrictions_ F16 #-}
{-# SPECIALIZE linearSubsRestrictions :: (VarIdx, LinComb' VarIdx F20) -> Restrictions_ F20 -> Restrictions_ F20 #-}
{-# SPECIALIZE linearSubsRestrictions :: (VarIdx, LinComb' VarIdx F24) -> Restrictions_ F24 -> Restrictions_ F24 #-}
...
printSolution :: Field field => Solution Name field -> IO ()
printSolution sol = 
  case sol of
...
analyzeResult :: Field field => Set VarIdx -> Result field -> Solution VarIdx field
...
data Result field
  = Finished   !(Restrictions_ field)     -- ^ we eliminated all equations (but there could be free variables!)
  | Unfinished !(Partial_      field)     -- ^ we couldn't eliminate all equations (solver didn't finish)
  deriving Show

printResult :: Field field => Result field -> IO ()
printResult result = 
  case result of
    Finished restr -> do
...
applyAction :: Field field => Action field -> Partial_ field -> Partial_ field
...
data Actions field
  = Single !(Action field)
  | Branch !(Action field) (Action field)
  deriving (Eq,Show)

instance Ord field => Ord (Actions field) where
...
branch :: Eq field => Action field -> Action field -> Actions field
...
checkSingleEquation :: Field field => R1CS' VarIdx field -> Actions field
...
solveLinearEquation :: Field field => LinComb' VarIdx field -> Action field
...
productIsZero :: Field field => LinComb' VarIdx field -> LinComb' VarIdx field -> Actions field
productIsZero aa bb = branch (solveLinearEquation aa) (solveLinearEquation bb) 

quadraticUnivariate :: Field field => R1CS' VarIdx field -> Actions field
...
noFreeVariables :: Field field => R1CS' VarIdx field -> Action field
...
data UniLinearEq field
  = UniLinearEq !field !field 
  deriving Show

-- | The univariate quadratic equation @a*x^2 + b*x+c = 0@
data UniQuadraticEq field 
  = UniQuadraticEq !field !field !field
  deriving Show

toUniLinearEq :: Field field => LinComb' VarIdx field -> (Maybe VarIdx, UniLinearEq field)
...
toUniQuadraticEq :: Field field => R1CS' VarIdx field -> (VarIdx, UniQuadraticEq field)
...
solveUniQuadratic :: Field field => UniQuadraticEq field -> Sqrt field
